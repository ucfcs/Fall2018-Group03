...
*              (Adapted to store literals using Delta Coding)
*
*   File    : lzss_deltas.c
*   Purpose : Use lzss coding (Storer and Szymanski's modified lz77) with the
*             addition of delta-encoded literals to compress/decompress files.
*   Authors : Michael Dipperstein - original LZSS code,
*             Isaac Ehlers - Delta Coding adaptation
*   Date    : November 24, 2003 (original LZSS code)
*
****************************************************************************
*   UPDATES
*
*   Date        Change
*   04/14/19    Added delta coding.
...
void EncodeLZSS(FILE *inFile, FILE *outFile)
{
    ...
    unsigned char prevUncoded;      /* value of previous uncoded symbol */
    unsigned char delta;            /* diff btwn following uncoded symbols */
    ...
    ************************************************************************/
    ...
    while (len > 0)
    {
        ...
        if (matchData.length <= MAX_UNCODED)
        {
            ...
            delta = (uncodedLookahead[uncodedHead] - prevUncoded) % 256;
            prevUncoded = uncodedLookahead[uncodedHead];
            encodedData[nextEncoded++] = delta;
        }
        else
        {
...
void DecodeLZSS(FILE *inFile, FILE *outFile)
{
    ...
    ************************************************************************/
    ...
    while (TRUE)
    {
        ...
        if (flags & 0x01)
        {
            /* delta-encoded character */
            if ((c = getc(inFile)) == EOF)
            {
                break;
            }

            /* decode delta-encoded character */
            delta = c;
            c = (delta + prevUncoded) % 256;
            prevUncoded = c;
